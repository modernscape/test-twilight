    ...
    <main className="relative h-[100dvh] w-full bg-white overflow-hidden flex flex-col font-sans">
      ...
      <AnimatePresence>
        {!isOpen && (
          ...
          >
            <span className="text-[10px] font-black leading-none block tracking-[0.2em]">MENU</span>
          </motion.button>
        )}
      </AnimatePresence>
          ...
          <>
            ...
            >
              <div className="flex items-start justify-between border-b border-black">
                <div className="p-6">
                  <Image src={`${basePath}/logo.png`} alt="logo" width={110} height={35} className="w-auto h-7 object-contain" />
                </div>
                <button
                  onClick={() => setIsOpen(false)}
                  className="p-6 md:p-10 border-l border-black flex flex-col items-center justify-center space-y-2 group bg-white"
                >
